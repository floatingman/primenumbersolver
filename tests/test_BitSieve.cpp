...
// Test that the bit sieve correctly identifies small primes
TEST_F(BitSieveTest, IdentifiesSmallPrimes) {
    BitSieve sieve(30);
    sieve.generate();
...
TEST_F(BitSieveTest, IsPrimeCorrectness) {
    BitSieve sieve(100);
    sieve.generate();
...
TEST_F(BitSieveTest, PrimeCountCorrectness) {
    BitSieve sieve(100);
    sieve.generate();
...
TEST_F(BitSieveTest, LargerLimit) {
    BitSieve sieve(1000);
    sieve.generate();
...
TEST_F(BitSieveTest, ExceedsLimit) {
    BitSieve sieve(100);
    sieve.generate();
...
TEST_F(BitSieveTest, NoRegeneration) {
    BitSieve sieve(100);
...
TEST_F(BitSieveTest, FileSaving) {
    BitSieve sieve(30);
    sieve.generate();
    
    std::string filename = "test_bit_primes.txt";
...
TEST_F(BitSieveTest, IsGenerated) {
    BitSieve sieve(100);
...
// Test memory usage
TEST_F(BitSieveTest, MemoryUsage) {
    BitSieve sieve(1000);
    sieve.generate();
    
    // Calculate expected memory usage
    std::size_t expectedSize = (1000 + 1 + 63) / 64;  // Number of uint64_t values
    std::size_t expectedMemory = expectedSize * sizeof(uint64_t);
    
    ASSERT_EQ(sieve.getMemoryUsage(), expectedMemory);
}

// Test that BitSieve produces the same results as BasicSieve
TEST_F(BitSieveTest, CompareWithBasicSieve) {
    ...
    ASSERT_EQ(bitSieve.getPrimeCount(), basicSieve.getPrimeCount());
    
    // Compare prime lists
    std::vector<std::size_t> bitPrimes = bitSieve.getPrimes();
    std::vector<std::size_t> basicPrimes = basicSieve.getPrimes();
    
    ASSERT_EQ(bitPrimes, basicPrimes);
}
